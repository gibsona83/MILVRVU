...
def load_data(file):
    ...
    try:
        ...
        # Debugging: Print the first 5 values before conversion
        if "Turnaround Time" in rvu_df.columns:
            sample_times = rvu_df["Turnaround Time"].dropna().astype(str).head(5).tolist()
            st.write(f"🔍 Debug: Sample Turnaround Time Values → {sample_times}")
        ...
        st.write(f"⏱️ Processing completed in {round(end_time - start_time, 2)} seconds.")
